...
var IQB = {
	 ...
	 * 下拉框字典表渲染
	 */
	getDictListByDictType2: function(selectId, dictType){
		var req_data = {'dictTypeCode': dictType};
		IQB.post(urls['rootUrl'] + '/sysDictRest/selectSysDictByType', req_data, function(result){
			$('select[name="'+selectId+'"]').prepend("<option value=''>请选择</option>");
			$('select[name="'+selectId+'"]').select2({theme: 'bootstrap', data: result.iqbResult.result});
			return result.iqbResult.result;
		})
	},
	/**
	...
	formatterDictTypeT: function(val, dictType){
		var req_data = {'dictTypeCode': dictType};
		var text = '';
		IQB.postAsync(urls['rootUrl'] + '/sysDictRest/selectSysDictByType', req_data, function(result){
			var retArr = result.iqbResult.result;
	...
	/*json转url*/
	jsonToUrlParams :function(json, key){ 
		  var urlParams=""; 
		  if(json instanceof String||json instanceof Number||json instanceof Boolean){ 
			  urlParams+="&"+key+"="+encodeURIComponent(json); 
		  }else{ 
		    $.each(json,function(i){ 
		      var k=key==null?i:key+(json instanceof Array?"["+i+"]":"."+i); 
		      urlParams+='&'+ IQB.jsonToUrlParams(this, k); 
		    }); 
		  } 
		  return urlParams.substr(1); 
	},
	/*重新登录*/
	toLogin: function(){
		//跳转首页
	...
	/*AJAX不封装返回结果success为2的情况*/
	ajaxIfFail: function(url, type, async, option, callback){
		var pendingRequests = {};
	...
	postIfFail: function(url, option, callback){
		IQB.ajaxIfFail(url, 'post', true, option, function(data){
			if($.isFunction(callback)){
	 			callback(data);
	 		}		
		});
	...
	$.extend($.fn.validatebox.defaults.rules, {	
	    ...
	    phone: {
	    	validator: function(value, param){
	    		...
	    		var mobPattern =  /^(13[0-9]|14[0-9]|15[0-9]|16[0-9]|17[0-9]|18[0-9]|19[0-9])\d{8}?$/;//手机号
	    ...
	    mobilePhone: {
	    	validator: function(value, param){
	    		var pattern = /^(13[0-9]|14[0-9]|15[0-9]|16[0-9]|17[0-9]|18[0-9]|19[0-9])\d{8}?$/ ;//手机号
	            return pattern.test(value);
			},
			message: '请输入真实的手机号'
	    },
