...
IQB.phoneRemember = function(){
	...
	var _this = {
		...
		config: {
			action: {//页面请求参数
				exports: urls['cfm'] + '/instRemindPhone/exportInstRemindPhoneList3'
  			},
		...
		exports : function(){
			...
			var datas = "?merchNames=" + merchName + "&orderId=" + orderId + "&realName=" + realName
			            ...
			            + "&processStartDate=" + processStartDate + "&processEndDate=" + processEndDate + "&code=" + code + "&flag=" + 1;
		...
		detail : function(){
			...
			if(records.length > 0){
				$('#open-win').modal('show');
				//datagrid2
				$("#datagrid2").datagrid2({url: urls['cfm'] + '/instRemindPhone/queryRecordList',paginator:'paginator2',queryParams : $.extend({}, {'orderId':records[0].orderId,'flag':1,'curItems':records[0].curItems})});
			}else{
				IQB.alert('未选中行');
			}
