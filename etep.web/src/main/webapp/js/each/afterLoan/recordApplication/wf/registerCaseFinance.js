...
$package('IQB.registerCaseFinance');
IQB.registerCaseFinance = function(){
	var _this = {
		cache: {			
			viewer: {},
			i: 1
		...
		openApproveWin: function(){
			if($('#updateForm').form('validate')){
				var option = {
						...
						'receivedDate': $('#arriveDate').val(),
						'receivedAmt': $('#arriveMoney').val(),
						'flag':'2'
				}
				//保存
				IQB.post(urls['cfm'] + '/afterLoan/updateCarReceiveByOrderId', option, function(result){
					if(result.success == '1'){
