...
$package('IQB.StorekeeperConfirmView');
IQB.StorekeeperConfirmView = function(){
	var _this = {
		cache: {			
			viewer: {}
		},	
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/afterLoan/getOrderInfo', {orderId: window.procBizId}, function(result){
				...
				$('#key').text(Formatter.yOrn(result.carKeyFlag));
		...
		showFile: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [35,36,101]}, function(result){
				var is = false;
...
$(function(){
	IQB.StorekeeperConfirmView.init();
});		