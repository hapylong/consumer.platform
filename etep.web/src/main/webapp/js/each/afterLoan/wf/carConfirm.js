...
$package('IQB.carConfirm');
IQB.carConfirm = function(){
	var _this = {
		cache: {			
			viewer: {}
		},	
		...
		openApproveWin: function(){
				...
				if($("#repaymentFlag").val()!="1"){
					...
					if($("#lostContactFlag").val()=="1"){
						if($("#lostContactFlag").val()=="1" && $("#lostContactReason").val()==""){
							IQB.alert("请输入失联原因");
							return false;
						}
					};
					if($("#lostContactFlag").val()=="2"){
						if($("#collectOpinionConfirm").val()==""){
							IQB.alert("请输入收车情况确认");
							  return false;
						}
					};
				...
				var option = {
						...
						'collectOpinionConfirm':$('#collectOpinionConfirm').val(),
