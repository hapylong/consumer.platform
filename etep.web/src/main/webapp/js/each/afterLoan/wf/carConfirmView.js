...
$package('IQB.carConfirmView');
IQB.carConfirmView = function(){
	var _this = {
		cache: {			
			viewer: {}
		},	
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/afterLoan/getOrderInfo', {orderId: window.procBizId}, function(result){
				...
				$('#trailerReason').text(Formatter.isNull(result.trailerReason));
				...
				$('#collectOpinionConfirm').text(Formatter.isNull(result.collectOpinionConfirm));
				$('#afterLoanOpinion').text(Formatter.isNull(result.afterLoanOpinion));
				$('#gpsArea').text(Formatter.isNull(result.gpsArea));
				$('#collectOpinion').text(Formatter.isNull(result.collectOpinion));
				if(result.lostContactFlag=='1'){
					$(".reasonBox").show();
					$(".carBox").hide();
				}
				else if(result.lostContactFlag=='2'){
					$(".reasonBox").hide();
					$(".carBox").show();
				}
			});
