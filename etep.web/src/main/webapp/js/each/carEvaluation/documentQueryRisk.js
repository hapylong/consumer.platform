$package('IQB.documentQueryRisk');
IQB.documentQueryRisk = function(){
	var _grid = null;
	var _tree = null;
	var _this = {
		cache :{
		...
		riskReport : function(orderId){
			IQB.post(urls['cfm'] + '/afterLoan/getReportList', {'orderId': orderId}, function(result){
				...
				if(result.success == 1){
					$.each(result.iqbResult.result,function(i,n){
						riskHtml += '<a class="riskReport" onclick="IQB.documentQueryRisk.showRiskReport('+n.reportType+')">'+n.reportName+'</a>';
					});
				}else{
