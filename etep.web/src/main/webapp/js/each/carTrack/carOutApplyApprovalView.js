...
console.log(procBizId)
var procInstId = getQueryString('procInstId');

$package('IQB.carOutApplyApprovalView');
IQB.carOutApplyApprovalView = function() {
    var _this = {
        cache: {
        	viewer: {}
        },
        ...
        showHtml: function() {
            ...
            IQB.post(urls['cfm'] + '/carstatus/cget_info' , data,function(result){
            	console.log(result.iqbResult.result.merchantShortName)
                ...
                $('#lostContactReason').text(Formatter.isNull(result.iqbResult.result.lostContactReason));
            });
        },
       
        showFile: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [29,30]}, function(result){
				var is = false;
        ...
        init: function(){       
			...
			$('#btn-uploadTypeThirtieth').on('click', function(){$('#file').attr('fileType', 30);$('#file').click();});	
...
$(function() {
	//页面初始化
    IQB.carOutApplyApprovalView.init();
});
