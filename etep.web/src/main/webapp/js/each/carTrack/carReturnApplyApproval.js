...
$package('IQB.carReturnApplyApproval');
IQB.carReturnApplyApproval = function(){
	var _this = {
		cache: {
			viewer: {},
			i : 1
		...
		openApproveWin: function(){
			...
			if($('#td-33').find('div').length == 0){
				IQB.alert('打款截图信息不完善，无法审核');
				return false;
			};
			//保存信息
			var isCharge = $("#isCharge").val();
			var feePlan = $("#feePlan").val();
			...
			var data = {
				    ...
				    'returnInfo':{
				    	"isCharge":isCharge,
						"feePlan": feePlan,
						"returnAmonut": planAmt
				    }
			}
			IQB.post(urls['cfm'] + '/carstatus/saveReturnInfo', data, function(result){
				if(result.success == 1){
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				if(result.iqbResult != '' && result.iqbResult != null){
					...
					$('#monthInterest').text(result.monthInterest);
					$('#carNo').text(result.carNo);
					$('#plate').text(result.plate);
					//填写的信息回显
					var returnInfo = result.returnInfoMap;
					if(returnInfo != null && returnInfo != ''){
						$('#isCharge').val(returnInfo.isCharge);
						$('#feePlan').val(returnInfo.feePlan);
						$('#planAmt').val(returnInfo.returnAmonut);
					}
				}
			});
		},
		...
		showFile: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [33,34]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			...
						      			'<div class="caption">' +
						      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.carReturnApplyApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.imgPath + '" alt="' + option.imgName + '" style="width: 135px; height: 135px;" /></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.imgName + '</h5><h6><a filePath="' + option.imgPath + '" onclick="IQB.carReturnApplyApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
		...
		init: function() {
			...
			_this.showFile();
			$('#isCharge').on('change',function(){_this.isCharge()});
...
$(function() {
	IQB.carReturnApplyApproval.init();
});
