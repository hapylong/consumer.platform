...
//var procBizId = "CDHTC2002170630007";
$package('IQB.carReturnApplyApprovalView');
IQB.carReturnApplyApprovalView = function(){
	var _this = {
		cache: {
			viewer: {},
			i : 1
			...
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				if(result.iqbResult != '' && result.iqbResult != null){
					...
					if(returnInfo != null && returnInfo != ''){
						$('#isCharge').text(_this.formatterIsCharge(returnInfo.isCharge));
						$('#feePlan').text(returnInfo.feePlan);
						$('#planAmt').text(Formatter.money(returnInfo.returnAmonut));
					}
				}
			});
		},
		formatterIsCharge:function(val){
			if(val != null){
				if(val == 1){
					return '是';
				}else{
...
$(function() {
	IQB.carReturnApplyApprovalView.init();
});
