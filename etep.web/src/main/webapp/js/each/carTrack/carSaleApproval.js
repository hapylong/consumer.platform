...
IQB.carSaleApproval = function(){
	var _this = {
		...
		openApproveWin: function(){
			if($('#assessAmt').val() == ''){
				IQB.alert('门店评估金额为空，无法审核');
				return false;
			}
			if($('#td-30').find('div').length == 0){
				IQB.alert('车辆评估信息不完善，无法审核');
				return false;
			};
			...
			if($('#price1').val() == '' || $('#name1').val() == '' || $('#phone1').val() == ''){
			...
			list[i-1].dealerAmt = bondsTable.rows[i].cells[1].getElementsByTagName("INPUT")[0].value;
			list[i-1].dealerName = bondsTable.rows[i].cells[2].getElementsByTagName("INPUT")[0].value;
			list[i-1].dealerMobile = bondsTable.rows[i].cells[3].getElementsByTagName("INPUT")[0].value;
			}
			var assessAmt = $("#assessAmt").val();
			var remark = $("#remark").val();
			var data = {
					'assessAmt' : assessAmt,
					'saleRemark' : remark,
					'dealerEvaluatesInfo':list,
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					...
					//填写的信息回显
					$('#assessAmt').val(result.assessAmt);
					$('#remark').val(result.saleRemark);
					//信息
					var dealerEvaluatesInfo = result.dealerEvaluatesInfoList;
					if(dealerEvaluatesInfo != null){
						$('#price1').val(dealerEvaluatesInfo[0].dealerAmt);
						$('#name1').val(dealerEvaluatesInfo[0].dealerName);
						$('#phone1').val(dealerEvaluatesInfo[0].dealerMobile);
						var creditHtml = '';
						for(var i=1;i<dealerEvaluatesInfo.length;i++){
							_this.cache.i = i + 1;
							creditHtml += "<tr>"+
							"<td><input type='checkbox' name='checkbox'></td>"+
							"<td><input type='number' class='form-control input-sm' value='"+dealerEvaluatesInfo[i].dealerAmt+"' maxlength='20'></td>"+
							"<td><input type='text' class='form-control input-sm' value='"+dealerEvaluatesInfo[i].dealerName+"'></td>"+
							"<td><input type='text' class='form-control input-sm' maxlength='11' value='"+dealerEvaluatesInfo[i].dealerMobile+"'></td>"+
							"</tr>"
						}
				...
				$.each(result.iqbResult.result, function(i, n){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			...
						      			'<div class="caption">' +
						      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.carSaleApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
		...
		addTr : function(){
			IQB.confirm('确定要增加一行吗？',function(){
				...
				var trrHtml =   "<tr>"+	
						    		...
						    		"<td><input type='number' id='price"+_this.cache.i+"' name='bondsmaName"+_this.cache.i+"' class='form-control input-sm'></td>"+				    		
