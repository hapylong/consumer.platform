...
IQB.carSaleApproval = function(){
	var _this = {
		...
		openApproveWin: function(){
			...
			if(parseFloat($('#arriveAmt').val()) != parseFloat(_this.cache.dealAmt)){
				IQB.alert('到帐金额与处置金额不等，无法审核');
				return false;
			}
			//保存信息
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					...
					_this.cache.dealAmt = result.dealAmt;
					$("#arriveAmt").val(result.receivedAmt);
					$("#transNo").val(result.serialNumber);
					//信息
