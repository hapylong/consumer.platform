...
IQB.carSaleApproval = function(){
	var _this = {
		...
		openApproveWin: function(){
			//验证
			if($('#settleFlag').val() == ''){
				IQB.alert('请选择是否结清车秒贷');
				return false;
			}
			//保存信息
			var settleFlag = $("#settleFlag").val();
			var shouldPayAmt = $("#shouldRepay").val();
			var data = {
					"shouldPayAmt":shouldPayAmt,
					"clearCarloanFlag":settleFlag,
				    "orderId": window.procBizId
			}
			if(settleFlag == '1'){
				//选择结清车秒贷
				//加一接口判断主订单是否包含车秒贷
				IQB.post(urls['cfm'] + '/carstatus/validateExistCmd', {"orderId": window.procBizId}, function(result){
					if(result.iqbResult.result.flag == 'N'){
						IQB.alert('该车贷订单没有车秒贷，请重新选择！');
						return false;
				    }else{
				    	//保存
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					...
					$("#shouldRepay").val(result.clearCarloanFlag);
					$("#shouldRepay").html(Formatter.isNull1(result.shouldPayAmt));
					//信息
		...
		settleFlagChange : function(){
			var settleFlag = $('#settleFlag').val();
			if(settleFlag != ''){
				IQB.post(urls['cfm'] + '/carstatus/getCurrentBalance', {orderId: window.procBizId,'flag':settleFlag}, function(result){
					$('#shouldRepay').html(Formatter.money(result.iqbResult.result.currentBanlance));
					$("#shouldRepay").val(result.iqbResult.result.currentBanlance);
				})
			}else{
				$('#shouldRepay').html('');
			}
		...
		init: function() {
			...
			//是否结清车秒贷回显应还金额
			$('#settleFlag').on('change',function(){
				_this.settleFlagChange();
			});
