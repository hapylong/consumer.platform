...
IQB.carSaleApproval = function(){
	var _this = {
		...
		openApproveWin: function(){
			//验证
			if($('#hqAssessAmt').val() == ''){
				IQB.alert('总部评估金额为空，无法审核');
				return false;
			}
			if($('#hqCheckAmt').val() == ''){
				IQB.alert('总部核实金额为空，无法审核');
				return false;
			}
			//保存信息
			var hqAssessAmt = $("#hqAssessAmt").val();
			var hqCheckAmt = $("#hqCheckAmt").val();
			var hqRemark = $("#hqRemark").val();
			var data = {
					"hqEvaluatesAmt": hqAssessAmt,
				    "hqCheckAmt": hqCheckAmt,
				    "orderId": window.procBizId,
				    "evaluatesRemark": hqRemark
			}
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					...
					$("#hqAssessAmt").val(result.hqEvaluatesAmt);
					$("#hqCheckAmt").val(result.hqCheckAmt);
					$("#hqRemark").val(result.evaluatesRemark);
					//信息
