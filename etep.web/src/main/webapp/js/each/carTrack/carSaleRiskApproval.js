...
IQB.carSaleApproval = function(){
	var _this = {
		...
		openApproveWin: function(){
			//验证
			if($('#disposeAmt').val() == ''){
				IQB.alert('处置金额为空，无法审核');
				return false;
			}
			//保存信息
			var disposeAmt = $("#disposeAmt").val();
			var data = {
					"dealAmt": disposeAmt,
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/carstatus/selOrderInfo', {orderId: window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					...
					$("#settleFlag").text(Formatter.yOrn(result.clearCarloanFlag));
					$("#shouldRepay").text(Formatter.money(result.shouldPayAmt));
					$("#disposeAmt").val(Formatter.isNull1(result.dealAmt));
					//信息
