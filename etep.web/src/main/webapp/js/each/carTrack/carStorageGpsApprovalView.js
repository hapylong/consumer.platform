...
$package('IQB.carStorageGpsApprovalView');
IQB.carStorageGpsApprovalView = function() {
    var _this = {
        cache: {
        	viewer: {}
        },
        ...
        showHtml: function() {
            ...
            IQB.post(urls['cfm'] + '/carstatus/cget_info' , data,function(result){
                ...
                $('#trailerReason').text(Formatter.trailerReason(result.iqbResult.result.trailerReason));
                $('#intoGarageRemark').text(result.iqbResult.result.intoGarageRemark);
                $('#intoGarageDate').text(Formatter.timeCfm2(result.iqbResult.result.createTime));
            });
        },
...
$(function() {
	//页面初始化
    IQB.carStorageGpsApprovalView.init();
});
