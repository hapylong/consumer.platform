...
$package('IQB.businessApprovalCredit');
IQB.businessApprovalCredit = function(){
	var _this = {
		cache: {			
			viewer: {},
			preAmount:''
...
//				$('#monthInterest_c').text(resl.monthInterest);
		...
		initSelect: function(){//初始化select组件
			IQB.post(urls['cfm'] + '/workFlow/getMerAllPlan', {orderId: window.procBizId.substring(0,window.procBizId.length-1)}, function(result){
				var arry = [];
				...
				$.each(result.iqbResult.result, function(i, n){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			...
						      			'<div class="caption">' +
						      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.businessApprovalCredit.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.imgPath + '" alt="' + option.imgName + '" style="width: 135px; height: 135px;" /></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.imgName + '</h5><h6><a filePath="' + option.imgPath + '" onclick="IQB.businessApprovalCredit.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
...
$(function(){
	IQB.businessApprovalCredit.init();
