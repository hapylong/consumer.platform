...
IQB.riskApproval = function() {
	var _this = {
		...
		openApproveWin: function() {
			//保存电核意见
			IQB.post(urls['cfm'] + '/business/saveApprovalOpinion', {orderId: window.procBizId, 'telApprovalOpinion':$('#telOption').val()}, function(result){
				if(result.success == '1'){
		...
		getOption : function(){
			IQB.post(urls['cfm'] + '/business/getApprovalOpinion', {'orderId': window.procBizId}, function(result){
				...
				$('#telOption').val(result.telApprovalOpinion);
			})
		},
		init: function() {
			//标签页样式动态处理
