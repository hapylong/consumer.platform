...
IQB.riskDepartmentApproval = function() {
	var _this = {
				...
				if (approveForm.approveStatus == "1") {
					IQB.post(urls['cfm'] + '/afterLoan/getReportStateByOrderId', {'orderId': window.procBizId}, function(result){
						if(result.success == '0'){
							...
							variableData.isSublet = _this.cache.isSublet;
							var bizData = {}
							bizData.procBizId=procBizId;
							bizData.isSublet = _this.cache.isSublet;
							var procData = {}
		...
		openApproveWin: function() {
			//保存电核意见
			IQB.post(urls['cfm'] + '/business/saveApprovalOpinion', {orderId: window.procBizId, 'riskApprovalOpinion':$('#riskOption').val(), 'telApprovalOpinion':_this.cache.telApprovalOpinion}, function(result){
				if(result.success == '1'){
		...
		riskReport : function(){
			IQB.post(urls['cfm'] + '/afterLoan/getReportList', {'orderId': window.procBizId}, function(result){
				...
				if(result.success == 1){
					$.each(result.iqbResult.result,function(i,n){
						riskHtml += '<a class="riskReport" onclick="IQB.riskDepartmentApproval.showReport('+n.reportType+')">'+n.reportName+'</a>';
					});
				}else{
		...
		getOption : function(){
			IQB.post(urls['cfm'] + '/business/getApprovalOpinion', {'orderId': window.procBizId}, function(result){
				...
				_this.cache.telApprovalOpinion = result.telApprovalOpinion;
				$('#riskOption').val(result.riskApprovalOpinion);
			})
		},
		init: function() {
			//标签页样式动态处理
