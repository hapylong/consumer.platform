...
$package('IQB.creditApprovalView');
IQB.creditApprovalView = function(){
	var _this = {
		cache: {
			viewer: {}
		},	
		...
		initCredit: function(){
			IQB.post(urls['cfm'] + '/credit_pro/get_info_details_x', {orderId:window.procBizId+'X'}, function(result){
				...
				$('#planId_c').text(resl.planFullName);
...
//				$('#monthInterest_c').text(resl.monthInterest);
		...
		showPreFee : function(){
			IQB.get(urls['cfm'] + '/order/otherAmt/getOtherAmtList/false', {'orderId': window.procBizId+'X'}, function(result){
