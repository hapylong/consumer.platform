...
$package('IQB.riskApprovalFirst');
IQB.riskApprovalFirst = function() {
	var _this = {
		cache: {
			viewer: {},
			greenChannel : '',
		...
		approve: function() {
			...
			if (approveForm.approveStatus) {
				if (approveForm.approveStatus == "1") {
					IQB.post(urls['cfm'] + '/afterLoan/getReportStateByOrderId', {'orderId': window.procBizId}, function(result){
						if(result.success == '0'){
							...
							variableData.greenChannel = _this.cache.greenChannel;
							var bizData = {}
						...
						}else if(result.success == '110'){
							IQB.alert('报告未生成不可以审核,稍等再试');
							return false;
						}else if(result.success == '120'){
							IQB.alert('报告发失败,不能审核,请联系管理员');
							return false;
						}
					})
		...
		openApproveWin: function() {
			//保存风控初审意见
			var phoneCheckOption = $("#phoneCheckOption").val();
			var data = {
					"orderId": window.procBizId,
					"riskApprovalOpinion": "",
					"telApprovalOpinion": phoneCheckOption
			}
			IQB.post(urls['cfm'] + '/business/saveApprovalOpinion', data, function(result){
				if(result.success == 1){
		...
		initApprovalTask: function(){//初始化订单详情
			IQB.post(urls['cfm'] + '/workFlow/getArtificialCheck', {orderId: window.procBizId}, function(result){
				...
				_this.cache.greenChannel = result.greenChannel;
				_this.userRiskInfo();
			});
			IQB.post(urls['cfm'] + '/business/getApprovalOpinion', {orderId: window.procBizId}, function(result){
				...
				$('#phoneCheckOption').val(Formatter.ifNull(result.telApprovalOpinion));
			})
		},
		userRiskInfo : function() {
			IQB.post(urls['cfm'] + '/user/user_risk_info', {'regId': $('#regId').html(), 'riskType':3}, function(result){
				result = result.iqbResult.result;
				$("#idCard").html(result.idCard);
				$("#addProvince").html(result.addProvince);
				$("#contactName1").html(result.contactName1);
				$("#contactPhone1").html(result.contactPhone1);
				$("#contactName2").html(result.contactName2);
				$("#contactPhone2").html(result.contactPhone2);
				$("#marriedStatus2").html(result.marriedStatus);
			})
		},
			...
			IQB.get(urls['cfm'] + '/order/otherAmt/getOtherAmtList/false', {'orderId': window.procBizId}, function(result){
				...
				if(result != '' && result != null){
					if(result.preAmtFlag == 1){
						...
						$('#riskAmt').text(Formatter.money(result.riskAmt));
		...
		riskReport : function(){
			IQB.post(urls['cfm'] + '/afterLoan/getReportList', {'orderId': window.procBizId}, function(result){
				...
				if(result.success == 1){
					$.each(result.iqbResult.result,function(i,n){
						riskHtml += '<a class="riskReport" onclick="IQB.riskApprovalFirst.showReport('+n.reportType+')">'+n.reportName+'</a>';
					});
				}else{
