	...
	var _this = {
				...
				if (approveForm.approveStatus == "1") {
					IQB.post(urls['cfm'] + '/afterLoan/getReportStateByOrderId', {'orderId': window.procBizId}, function(result){
						if(result.success == '0'){
							...
							variableData.orderAmt =  parseFloat(_this.cache.orderAmt);
							var bizData = {}
							bizData.procBizId=procBizId;
							bizData.orderAmt = parseFloat(_this.cache.orderAmt);
							var procData = {}
		...
		riskReport : function(){
			IQB.post(urls['cfm'] + '/afterLoan/getReportList', {'orderId': window.procBizId}, function(result){
				...
				if(result.success == 1){
					$.each(result.iqbResult.result,function(i,n){
						riskHtml += '<a class="riskReport" onclick="IQB.riskApproval.showReport('+n.reportType+')">'+n.reportName+'</a>';
					});
				}else{
