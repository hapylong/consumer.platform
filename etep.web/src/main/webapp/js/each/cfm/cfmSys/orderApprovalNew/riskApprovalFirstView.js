...
$package('IQB.riskApprovalFirstView');
IQB.riskApprovalFirstView = function() {
	var _this = {
		cache: {
			viewer: {},
			greenChannel : '',
		...
		riskReport : function(){
			IQB.post(urls['cfm'] + '/afterLoan/getReportList', {'orderId': window.procBizId}, function(result){
				...
				if(result.success == 1){
					$.each(result.iqbResult.result,function(i,n){
						riskHtml += '<a class="riskReport" onclick="IQB.riskApprovalFirstView.showReport('+n.reportType+')">'+n.reportName+'</a>';
					});
				}else{
...
$(function() {
	IQB.riskApprovalFirstView.init();
});
