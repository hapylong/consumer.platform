$package('IQB.telephoneRemind');
IQB.telephoneRemind = function(){
	var _grid = null;
	var _tree = null;
    var merchantFlag= 1;
    var showDataHtml="";
	var _this = {
		...
		reset : function(){
			$("#btn-reset").click(function(){
				...
				$('#select2-failReason-container').text('请选择');
				$('#select2-mobileDealOpinion-container').text('请选择');
				$('#select2-telRecord-container').text('请选择');
			})
		},
		config: {
			action: {//页面请求参数
  				exports: urls['cfm'] + '/instRemindPhone/exportInstRemindPhoneList',
  			},
			event: {//按钮绑定函数，不定义的话采用默认
  			...
  			dataGrid: {//表格参数  				
  				url: urls['cfm'] + '/instRemindPhone/queryList',
	   			queryParams: {flag: '1','merchNames':'全部商户'},
	   			singleCheck:true,
		...
		exports : function(){
			$("#btn-export").click(function(){
				...
				var datas = "?merchNames=" + merchNames + "&telRecord=" + telRecord + "&orderId=" + orderId + "&regId=" + regId + "&realName=" + realName + "&startDate=" + startDate + "&endDate=" + endDate+ "&failReason=" + failReason + "&mobileDealOpinion=" + mobileDealOpinion+ "&flag=" + 1 ; 
			...
			$("#btn-insert1").click(function(){
				  ...
				  if(rows.length > 0){
						if(rows.length > 1){
							...
							window.parent.IQB.main2.openTab("remind", "电话提醒", encodeURI('/etep.web/view/cfm/cfmSys/orderApprovalNew/telephoneRemindAct.html?orderId=' + rows[0].orderId+ '&curItems=' + rows[0].curItems), true, true, {lastTab: param});
						}
					}else{
						IQB.alert('未选中行');
					} 
			...
			$("#btn-update1").click(function(){
				...
				if(rows.length > 0){
					if(rows.length > 1){
						  ...
						  var rows = $("#datagrid").datagrid2('getCheckedRows');
						  window.parent.IQB.main2.openTab("remindDetail", "电话提醒详情", encodeURI('/etep.web/view/cfm/cfmSys/orderApprovalNew/telephoneRemindDetail.html?orderId=' + rows[0].orderId+ '&merchantName=' + rows[0].merchantName+ '&realName=' + rows[0].realName+ '&regId=' + rows[0].regId), true, true, null);  
					}
				}else{
					IQB.alert('未选中行');
				}
		...
		riskInfoShow : function(){
			var rows = $("#datagrid").datagrid2('getCheckedRows');
			if(rows.length > 0){
				IQB.post(urls['cfm'] + '/instRemindPhone/getRiskInfoByOrderId', {"orderId":rows[0].orderId,"flag":1}, function(result){
					if(result.success == '1'){
        ...
        refresh : function(page){
			$("#datagrid").datagrid2({url: urls['cfm'] + '/instRemindPhone/queryList',queryParams : $.extend({},{flag: '1'},$("#searchForm").serializeObject(), {pageNum: _this.cache.page})});
	    },
		init: function(){ 
