	...
	var _this = {
		...
		approve: function() {
			...
			if (approveForm.approveStatus) {
				...
				bizData.receiveAmt=$('#paymentAmt').val();
				bizData.recieveDate=$('#paymentTime').val();
				var procData = {}
		...
		openApproveWin: function() {
			if($('#td-1').find('div').length == 0){
				IQB.alert('收款截图为空，无法审核');
				return false;
			};
			if($('#updateForm').form('validate')){
				var data = {
						'orderId':window.procBizId,
						'receiveAmt':$('#paymentAmt').val(),
						'recieveDate':$('#paymentTime').val(),
						'payMethod':_this.cache.payMethod
				}
		...
		initApprovalTask: function(){//初始化详情
			IQB.post(urls['cfm'] + '/settle/getSettleBean', {orderId: window.procBizId}, function(result){
				...
				$('#paymentAmt').val(Formatter.money(result.receiveAmt));
				$('#paymentTime').val(result.recieveDate);
				//支付状态
		...
		showFile: function(){			
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [1,2]}, function(result){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						if(n.imgType == 1){
							...
							var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
							      			...
							      			'<div class="caption">' +
							      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.financeApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
							      			'</div>' + 
							      		'</div>';
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.imgPath + '" alt="' + option.imgName + '" style="width: 135px; height: 135px;" /></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.imgName + '</h5><h6><a filePath="' + option.imgPath + '" onclick="IQB.financeApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
