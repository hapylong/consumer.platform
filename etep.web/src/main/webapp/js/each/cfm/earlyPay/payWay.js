...
IQB.payWay = function(){
	...
	var _this = {
		...
		//获取信息
		getValue : function(){
			var amountObject = localStorage.getItem("amountObject");
			object = JSON.parse(amountObject);
			$("input[name='orderId']").val(object.orderId);
			$("input[name='amount']").val(object.amount);
			$("input[name='id']").val(object.id);
			$("input[name='payList']").val('[{\"orderId\":\"'+object.orderId+'\",\"regId\":\"'+object.regId+'\",\"repayModel\":\"all\",\"sumAmt\":'+object.sumAmt+'}]');
		},
		//网银
		...
		config: {
			action: {//页面请求参数
				...
				onlinePay: urls['cfm'] + '/pay/paySettlementByCashierDesk',//网银支付
				fastPay: urls['cfm'] + '/pay/paySettlementByQuick/all'//快捷支付
			},
