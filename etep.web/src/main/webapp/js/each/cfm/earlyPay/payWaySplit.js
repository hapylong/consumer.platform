...
IQB.payWay = function(){
	...
	var _this = {
		...
		//获取信息
		getValue : function(){
			var payWaySplit = localStorage.getItem("objectSplit");
			payWaySplit = JSON.parse(payWaySplit);
			console.log(payWaySplit);
			$("input[name='orderId']").val(payWaySplit.orderId);
			$("input[name='amount']").val(payWaySplit.amount);
			$("input[name='id']").val(payWaySplit.id);
			$("input[name='payList']").val('[{\"orderId\":\"'+payWaySplit.orderId+'\",\"regId\":\"'+payWaySplit.regId+'\",\"repayModel\":\"all\",\"sumAmt\":'+payWaySplit.sumAmt+'}]');
		},
		toPayFast : function(){
		...
		config: {
			action: {//页面请求参数
				...
				fastPay: urls['cfm'] + '/pay/paySettlementByQuick/break'//快捷支付
			},
