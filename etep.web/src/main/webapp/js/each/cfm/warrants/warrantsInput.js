...
$package('IQB.warrantsInput');
IQB.warrantsInput = function(){
	var _this = {
		cache: {
			viewer: {}
		},
			...
			IQB.post(urls['cfm'] + '/workFlow/getArtificialCheck', {'orderId': window.procBizId}, function(result){
				...
				IQB.post(urls['cfm'] + '/authoritycard/querryByOrderId', {'orderId': window.procBizId}, function(result){
					if(result.iqbResult.AuthorityCardInfo != null){
						...
						$("#inCarTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.getCarDate));
						$("#annualInspectionTtime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.checkDate));
						$("#GPSTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.gpsInstDate));
						$("#insuranceStartTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.insuranceStart));
						$("#insuranceEndTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.insuranceEnd));
						...
						$("#commercialInsuranceStartTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.bizRisksStart));
						$("#commercialInsuranceEndTime").val(Formatter.timeCfm5(result.iqbResult.AuthorityCardInfo.bizRisksEnd));
				...
				$.each(result.iqbResult.result, function(i, n){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			...
						      			'<div class="caption">' +
						      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.warrantsInput.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
