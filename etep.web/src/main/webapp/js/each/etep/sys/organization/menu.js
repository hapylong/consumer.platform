...
IQB.menu = function(){
	...
	var _this = {
		...
		check: function(treeNodes){//自定义函数(部分勾选)
			...
			if(treeNodes && treeNodes.length > 0){
				...
				$.each(treeNodes, function(i, m){		
					var treeNode = treeObj.getNodeByParam('id', m);
		...
		config: {
			action: {//页面请求参数
  				insert: urls['rootUrl'] + '/sysOrganizationRest/insertSysOrganizationPurview', 
  				get: urls['rootUrl'] + '/sysOrganizationRest/getSysOrganizationPurview'
  			},
			event: {//按钮绑定函数
				save: function(){//重写save
					...
					if(organizationTreeNodes && organizationTreeNodes.length > 0){						
						var option = {};
						option.orgId = organizationTreeNodes[0].id;						
						var menuTreeNodes = _menuTree.getTreeObj().getCheckedNodes();
						if(menuTreeNodes && menuTreeNodes.length > 0){
							var newMenuIds = new Array();
							$.each(menuTreeNodes, function(i, m){	
								newMenuIds.push(m.id);							
							});	
							option.newMenuIds = newMenuIds;		
						}						
						IQB.post(_this.config.action.insert, option, function(result){         
							IQB.alert('保存成功');	
						});
					}else{
						IQB.alert('未选中机构');
