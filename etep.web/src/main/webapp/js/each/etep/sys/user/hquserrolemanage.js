	...
	var _this = {
			...
			if (_box.util.checkSelectOne(records)) {
				if (records[0].status == 1) {
					...
					IQB.get(_this.config.action.getRoleInfoByOrgIdAndHq, data, function(result) {
						$('#roleUpdate').select2({theme : 'bootstrap',data : result.iqbResult.result,maximumSelectionLength: 20,placeholder: "请选择"});
						$('#roleUpdate').val(arr);
						$('#roleUpdate').change();
					})
				} else {
		...
		initOrgSelect : function() {// 初始化机构下拉
			IQB.get(_this.config.action.getOrgInfo, {}, function(result) {
				...
				$('#orgIdSelectAdd').select2({theme : 'bootstrap',data : result.iqbResult.result}).on('change',  function() {_this.initDeptSelect($('#orgIdSelectAdd').val());});
