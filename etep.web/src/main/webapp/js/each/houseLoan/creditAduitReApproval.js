...
$package('IQB.creditAduitReApproval');
IQB.creditAduitReApproval = function(){
	var _this = {
		cache: {
			viewer: {},
			i:1,
		...
		initApprovalTask: function(){//初始化订单详情
			...
			IQB.post(urls['cfm'] + '/dandelion/get_info', {orderId: window.procBizId}, function(result){
				...
				IQB.post(urls['cfm'] + '/credit_pro/plan_details_by_criteria', option, function(resultPlan){
					var result = resultPlan.iqbResult.result;
					var planHtml = "<option value=''>请选择</option>";
					for(var i=0;i<result.length;i++){
						planHtml += "<option value='"+result[i].id+"'>"+result[i].planFullName+"</option>";
					}
					$('#plan').find('option').remove();
					$('#plan').append(planHtml);
					//plan回显
		...
		showFile2: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [5,6,7,8]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					var extensionName = Formatter.getExtensionName(n.imgName);
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + n.imgPath + '" alt="' + n.imgName + '" style="width: 135px; height: 135px;" /></a>' +
						      			'<div class="caption">' +
						      				'<h5>' + n.imgName + '</h5><h6><a filePath="' + n.imgPath + '" onclick="IQB.creditAduitReApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
						$('#td-' + n.imgType).append(html);
						is = true;
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.imgPath + '" alt="' + option.imgName + '" style="width: 135px; height: 135px;" /></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.imgName + '</h5><h6><a filePath="' + option.imgPath + '" onclick="IQB.creditAduitReApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
		...
		showInfo : function(){
			IQB.post(urls['cfm'] + '/dandelion/get_designated_person_info', {orderId: window.procBizId}, function(result){
				...
				for(var i=0;i<creditInfo.length;i++){
					...
					creditOption += "<option value='"+creditInfo[i].loanAmt+"'>"+creditInfo[i].loanAmt+"</option>";
				}
				$('#loanTable').append(creditHtml);
				$('#lendAmt').append(creditOption);
				if(result.orderAmt != '' && result.orderAmt != null){
					$('#lendAmt').val(result.orderAmt);
				}
				//担保人信息
...
$(function() {
	IQB.creditAduitReApproval.init();
});
