...
IQB.carLoanDeptRisk = function() {
    var _this = {
		...
		showFile3: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [24]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					var extensionName = Formatter.getExtensionName(n.imgName);
					if(Formatter.extensionName.doc.contain(extensionName)){
        ...
        init: function(){ 
        	...
        	$('#btn-uploadType24').on('click', function(){$('#file').attr('fileType', 24);$('#file').click();});	
