...
IQB.carLoanDeptRisk = function() {
    var _this = {
		...
		showFile2: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [25,26,27,28,29,30]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					var extensionName = Formatter.getExtensionName(n.imgName);
					if(Formatter.extensionName.doc.contain(extensionName)){
        ...
        showHtml: function() {
        	...
        	IQB.post(urls['cfm'] + '/ownerloan/getMortgageInfo' , data,function(result){
        		var result = result.iqbResult.result;
            	$('#suggestAmt').val(Formatter.isNull(Formatter. moneyTs(result.suggestAmt))); 
            	$('#suggestItems').val(Formatter.isNull(result.suggestItems)); 
            	$('#storeRiskAdvice').val(Formatter.isNull(result.storeRiskAdvice));  
            });
        },
        openApproveWin: function(){
        	if($('#updateForm1').form('validate')){
        		if(Number(Formatter.removeMoneyFormatter($('#suggestAmt').val()))>50000){
        			IQB.alert('建议审批额度小于等于5万');
					return false;
        		}
        		if($('#td-25').find('div').length == 0){
					IQB.alert('车辆基本信息未上传图片，无法审核');
					return false;
				};
				if($('#td-26').find('div').length == 0){
					IQB.alert('车辆抵押记录未上传图片，无法审核');
					return false;
				};
				if($('#td-27').find('div').length == 0){
					IQB.alert('车辆违章情况未上传图片，无法审核');
					return false;
				};
				if($('#td-28').find('div').length == 0){
					IQB.alert('被执行信息未上传图片，无法审核');
					return false;
				};
				if($('#td-29').find('div').length == 0){
					IQB.alert('失信信息未上传图片，无法审核');
					return false;
				};
				var data = {
					'orderId':procBizId,	
					'suggestAmt':Formatter.removeMoneyFormatter($('#suggestAmt').val()),
					'suggestItems':$('#suggestItems').val(),
					'storeRiskAdvice':$('#storeRiskAdvice').val(),
				}
				IQB.post(urls['cfm'] + '/ownerloan/updateMortgageInfo', data, function(result){
					if(result.success == 1){
		...
		formatterMoney : function(){
		    var suggestAmt = $('#suggestAmt').val();
		    $('#suggestAmt').val(Formatter.moneyTs(suggestAmt));
		},
        init: function(){ 
        	_this.showHtml(); 
        	_this.showFile();
        	_this.showFile1();
        	_this.showFile2();
        	$('#suggestAmt').on('blur',function(){_this.formatterMoney()});
        	$('#btn-approve').on('click', function(){_this.openApproveWin()});
			$('#btn-unassign').on('click', function(){_this.unassign()});
			$('#btn-approve-save').on('click', function(){_this.approve()});
			$('#btn-approve-close').on('click', function(){_this.closeApproveWin()});
        	$('#btn-uploadType25').on('click', function(){$('#file').attr('fileType', 25);$('#file').click();});	
			$('#btn-uploadType26').on('click', function(){$('#file').attr('fileType', 26);$('#file').click();});
			$('#btn-uploadType27').on('click', function(){$('#file').attr('fileType', 27);$('#file').click();});
			$('#btn-uploadType28').on('click', function(){$('#file').attr('fileType', 28);$('#file').click();});
			$('#btn-uploadType29').on('click', function(){$('#file').attr('fileType', 29);$('#file').click();});
			$('#btn-uploadType30').on('click', function(){$('#file').attr('fileType', 30);$('#file').click();});
