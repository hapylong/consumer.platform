...
IQB.carLoanDeptRisk = function() {
    var _this = {
		...
		showFile3: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [31,32,33,34,35,36,37,38,39]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					var extensionName = Formatter.getExtensionName(n.imgName);
					if(Formatter.extensionName.doc.contain(extensionName)){
        ...
        showHtml: function() {
        	...
        	IQB.post(urls['cfm'] + '/ownerloan/getDeptSignInfo' , data,function(result){
				...
				$('#preAmtStatus').text(Formatter.preAmountStatus(Formatter.isNull(result.preAmtStatus)));
        ...
        openApproveWin: function(){
        	if($('#updateForm').form('validate')){
        		if($('#td-31').find('div').length == 0){
					IQB.alert('借款合同未上传图片，无法审核');
					return false;
				};
				if($('#td-32').find('div').length == 0){
					IQB.alert('咨询服务协议未上传图片，无法审核');
					return false;
				};
				if($('#td-33').find('div').length == 0){
					IQB.alert('质押合同未上传图片，无法审核');
					return false;
				};
				if($('#td-34').find('div').length == 0){
					IQB.alert('出借合同未上传图片，无法审核');
					return false;
				};
				if($('#td-35').find('div').length == 0){
					IQB.alert('租赁合同未上传图片，无法审核');
					return false;
				};
				if($('#td-36').find('div').length == 0){
					IQB.alert('备用钥匙未上传图片，无法审核');
					return false;
				};
				if($('#td-37').find('div').length == 0){
					IQB.alert('放款申请书未上传图片，无法审核');
					return false;
				};
        ...
        init: function(){ 
			...
			$('#btn-uploadType31').on('click', function(){$('#file').attr('fileType', 31);$('#file').click();});
			$('#btn-uploadType32').on('click', function(){$('#file').attr('fileType', 32);$('#file').click();});
			$('#btn-uploadType33').on('click', function(){$('#file').attr('fileType', 33);$('#file').click();});
			$('#btn-uploadType34').on('click', function(){$('#file').attr('fileType', 34);$('#file').click();});
			...
			$('#btn-uploadType37').on('click', function(){$('#file').attr('fileType', 37);$('#file').click();});
        	$('#btn-uploadType38').on('click', function(){$('#file').attr('fileType', 38);$('#file').click();});	
