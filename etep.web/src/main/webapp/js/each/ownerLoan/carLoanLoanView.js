...
$package('IQB.carLoanLoanView');
IQB.carLoanLoanView = function() {
    var _this = {
        cache: {
        	viewer: {}
        }, 
		...
		showFile3: function(){
			IQB.post(urls['cfm'] + '/image/getImageList', {orderId: window.procBizId, imgType: [31,32,33,34,35,36,37,38,39,41,42]}, function(result){
				var is = false;
				$.each(result.iqbResult.result, function(i, n){
					var extensionName = Formatter.getExtensionName(n.imgName);
					if(Formatter.extensionName.doc.contain(extensionName)){
        ...
        showHtml: function() {
        	...
        	IQB.post(urls['cfm'] + '/ownerloan/getDeptSignInfo' , data,function(result){
				...
				$('#preAmtStatus').text(Formatter.isNull(result.preAmtStatus));
        	...
        	IQB.post(urls['cfm'] + '/ownerloan/getLoanInfo' , data,function(result){
            	...
            	$('#loanAmt').text(Formatter.money(result.loanAmt));
            	$('#loanDate').text(Formatter.isNull(result.loanDate));  
            });
        },
