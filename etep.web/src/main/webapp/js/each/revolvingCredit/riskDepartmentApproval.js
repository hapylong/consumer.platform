	...
	var _this = {
		...
		openApproveWin: function(){
			if($('#updateForm').form('validate')){
				if(Number($('#lendAmt').val()) > Number(_this.cache.financingLimit)){
					IQB.alert('批准额度应小于等于可融资额');
					return false;
				}
				var data = {
						...
						'bizType':'3002',
						'proType':'华益周转贷',
						'bankCardName':$('#bankCardName').text()
				}
		...
		initApprovalTask: function(){//初始化订单详情
			...
			IQB.post(urls['cfm'] + '/dandelion/get_info', {orderId: window.procBizId}, function(result){
				...
				//计划相关
				IQB.post(urls['cfm'] + '/credit_pro/plan_details_by_criteria', {'merchantNo':_this.cache.merchantNo,'bizType':'3002'}, function(resultPlan){
					var result = resultPlan.iqbResult.result;
					var planHtml = "<option value=''>请选择</option>";
					for(var i=0;i<result.length;i++){
						planHtml += "<option value='"+result[i].id+"'>"+result[i].planName+"</option>";
					}
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.imgPath + '" alt="' + option.imgName + '" style="width: 135px; height: 135px;" /></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.imgName + '</h5><h6><a filePath="' + option.imgPath + '" onclick="IQB.riskDepartmentApproval.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
		...
		showInfo : function(){
			IQB.post(urls['cfm'] + '/dandelion/get_designated_person_info', {orderId: window.procBizId}, function(result){
        		...
        		_this.cache.financingLimit = result.icie.adviceAmt;
				//担保人个数
		...
		cal:function(){
			if($('#lendAmt').val() != '' && $('#plan').val() != ''){
				...
				IQB.post(urls['cfm'] + '/dandelion/recalculate_amt', option, function(result){
					...
					$('#orderItems2').text(Formatter.ifNull(result.qs));
